...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('✪ Premium subscription', url="https://telegra.ph/Premium-subscription-Detail-08-02"),
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('Premium subscribe 💸', url="https://telegra.ph/Premium-subscription-Detail-08-02"),
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        n=await message.reply_photo("https://dragonkindom.jarvisjarvis517.workers.dev/0:/photo_2023-08-31_04-47-09_7273366770447647744.jpg").split()
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          ...
                          InlineKeyboardButton("💰 Premium Detail 💰", url="https://telegra.ph/Premium-subscription-Detail-08-02")
                         ]
                        ]
                    )
                )
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@st_movie_upadate  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
